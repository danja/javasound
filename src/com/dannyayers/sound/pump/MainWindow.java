...
public class MainWindow extends Window implements MouseMotionListener,
	...
	this.frame = frame;
        ...
        setVisible(true);
        addNotify();
	// Load the command line media, if any
        if (mediaURL != null)
            loadMovie(mediaURL);
     ...
     * Currently only a single media URL is supported
     */ 
     ...
     */
    private void loadMovie(String mediaURL) {
	// Prepend a "file:" if no protocol is specified
	if (mediaURL.indexOf(":") < 3)
    ...
    public static Image loadImage ( String strFileName ) {
        ...
        try {
            classObject = Class.forName ( "MainWindow" );// jamp.MainWindow
